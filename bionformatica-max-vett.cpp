...
public:
    ...
    {
        if (numnodi >= MAXN) {
            cout << "E' stato raggiunto il massimo limite dei nodi" << endl;
            exit(1);
        }
    ...
    {
        cout << origine << destinazione << endl;
        if (numarchi >= MAXA) {
            cout << "E' stato raggiunto il massimo limite degli archi" << endl;
            exit(1);
        }
        numarchi++;
        string i = origine.substr(1, 2); // TODO e se il nodo ha una cifr O PIù?
        cout << "i: " << (stoi(i)-1) << endl; // TODO due vettori paralleli, uno con l'origine uno con la destinazione
        adiacenze[stoi(i) - 1] = Arco(origine, destinazione, overlap);
    }
    ...
    void stampaGrafo()
    {
      ...
      // TODO fare un azzeramento del vettore adiacenze. AGG: forse non ce ne è bisogno, provare con il metodo empty
        for (int i = 0; i < numnodi; i++) {
            ...
            if (adiacenze[i].size() == 0) {
                cout << "non ci sono nodi collegati a " << nodi[i].id << endl;
            }
            for (int j = 0; j < adiacenze[i].size(); j++) {
                cout << (adiacenze[i][j]).destinazione << endl;
            }
            cout << endl;
        }
    }
